...
int main (void)
{
    ...
    int b_ticks = (PACER_RATE/BUTTON_TASK_RATE); // test
    int d_ticks = (PACER_RATE/DISPLAY_TASK_RATE);
    int t_ticks = (PACER_RATE/TIMER_TASK_RATE);
    int counter = 0;
    ...
    /* TODO: Implement the functionality of the tasks in the
           stopwatch1 program.  */
        if (counter % b_ticks == 0)
            button_task();
        if (counter % d_ticks == 0)
            display_task();
        if (counter % t_ticks == 0)
            timer_task();
